...
for (let claim of input) {
  // extract numbers from claim-string to array of numbers
  let claimNums = claim.match(/\d+/g).map(Number);
  // add overlapping inches from this claim
  overlappingTotal += claimFabric(fabric, claimNums);
}

function claimFabric(fabric, claim) {
  let overlapping = 0;
  // fill claim to fabric
  // claim = [elveID, leftGap, topGap, width, height]
